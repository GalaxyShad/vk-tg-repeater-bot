...
const telegramSendDocumentsFromVk = async (telegramBot, docList, author) => {
  ...
  return media.length > 1
    ? telegramBot
        .sendMediaGroup(telegram.CHAT_ID, media)
        .catch((err) => console.log(err.response.body))
    : telegramBot
        .sendDocument(telegram.CHAT_ID, media[0].media, {
          caption: author,
        })
...
const telegramSendPhotosFromVk = async (telegramBot, photoList, author) => {
  ...
  return media.length > 1
    ...
    : telegramBot
        .sendPhoto(telegram.CHAT_ID, media[0].media, { caption: author })
        .catch((err) => console.log(err.body));
};
...
const vkParseMessage = async (vk, vkMessage, depth = 0) => {
  ...
  str +=
    (await vkParseMessage(vk, reply_message, depth + 1)) +
...
vk.bot.on(async (ctx) => {
  ...
  if (respondText !== "")
    telegram.bot.sendMessage(telegram.CHAT_ID, respondText);
});

telegram.bot.on("message", async (msg) => {
  const { chat, photo, from } = msg;

  if (chat.id !== telegram.CHAT_ID) return;

  await vkSendPhoto(vk, photo, from).catch((err) => {
    console.log(err);
